...
public class SQLAccountManager
{
	...
	public static void main(String[] args) throws SQLException, IOException, NoSuchAlgorithmException
	{
        ...
        System.out.println("");
		System.out.println("1-新增或更改已存在的帳號(改變密碼和存取權限)");
		System.out.println("2-改變存取權限");
		System.out.println("3-刪除帳號");
		System.out.println("4-列出帳號和存取權限");		
	...
	{		
		...
		if(rset.next()==false) {
			...
			System.out.println("帳號(" + account + ")的資料已成功更新.");				
		} else {		
			// Not Exist
			System.out.println("帳號(" + account +" )的資料並不存在.");		
		}
		rset.close();	
