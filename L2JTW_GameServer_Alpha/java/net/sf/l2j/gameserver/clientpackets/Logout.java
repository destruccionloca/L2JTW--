...
public class Logout extends ClientBasePacket
{
	...
	player.getInventory().updateDatabase();
        ...
        if(player.atEvent) {
            player.sendPacket(SystemMessage.sendString("無法退出活動"));
            return;
        }
        ...
        if (player.isFestivalParticipant()) {
            ...
            {
                player.sendMessage("在黑暗祭典期間無法登出.");
                return;
            }
            L2Party playerParty = player.getParty();
            
            if (playerParty != null)
                player.getParty().broadcastToPartyMembers(SystemMessage.sendString(player.getName() + " 從黑暗祭典內移除."));
        }
        if (player.isFlying()) 
        { 
