...
public abstract class ItemContainer
{
    ...
    public L2ItemInstance addItem(String process, L2ItemInstance item, L2PcInstance actor, L2Object reference)
    {
		...
		{
			// prevent item quantity overflow
			int count = item.getCount();
			int curCount = olditem.getCount();
			if (curCount > Integer.MAX_VALUE - count)
			{
				actor.sendMessage("You reached the maximum number of item.");
				
				// drop items
				int newCount = Integer.MAX_VALUE - curCount;
				L2ItemInstance itemD = ItemTable.getInstance().createItem(process, olditem.getItemId(), count - newCount, actor, reference);
				itemD.dropMe(actor, actor.getClientX() + Rnd.get(50) - 25, actor.getClientY() + Rnd.get(50) - 25, actor.getClientZ() + 20);
				itemD.setDropTime(0);
    ...
    public L2ItemInstance addItem(String process, int itemId, int count, L2PcInstance actor, L2Object reference)
    {
		...
		if (item != null && item.isStackable())
		{
			...
			if (curCount > Integer.MAX_VALUE - count)
			{
				...
				// drop items
				int newCount = Integer.MAX_VALUE - curCount;
				L2ItemInstance itemD = ItemTable.getInstance().createItem(process, itemId, count - newCount, actor, reference);
				itemD.dropMe(actor, actor.getClientX() + Rnd.get(50) - 25, actor.getClientY() + Rnd.get(50) - 25, actor.getClientZ() + 20);
				itemD.setDropTime(0);
