...
public class AdminEditNpc implements IAdminCommandHandler {
    ...
    private void editShopItem(L2PcInstance admin, String[] args)
    {
        ...
        if (args.length > 3)
        {
            ...
            admin.sendMessage("更新價位 物品"+item.getName()+" 在編號 "+tradeListID);
        ...
        replyMSG.append("<html><title>商人販賣物品</title>");
        replyMSG.append("<body>");
        replyMSG.append("<br>修改登錄值在商人列表內.");
        replyMSG.append("<br>修改物品: "+item.getName());
    ...
    private void delShopItem(L2PcInstance admin, String[] args)
    {
        ...
        TextBuilder replyMSG = new TextBuilder();
        replyMSG.append("<html><title>商人販賣物品刪除</title>");
        replyMSG.append("<body>");
        replyMSG.append("<br>刪除登錄值在商人列表內.");
        replyMSG.append("<br>物品刪除: "+ItemTable.getInstance().getTemplate(itemID).getName());
        replyMSG.append("<table>");
    ...
    private void addShopItem(L2PcInstance admin, String[] args)
    {
        ...
        if (args.length > 3)
        {
            ...
            admin.sendMessage("增加 "+newItem.getItem().getName()+" 道交易名單 "+tradeList.getListId());
            showShopList(admin, tradeListID, 1);
            return;
        }
        ...
        TextBuilder replyMSG = new TextBuilder();
        replyMSG.append("<html><title>商人物品販賣增加</title>");
        replyMSG.append("<body>");
        replyMSG.append("<br>增加登錄值到商人列表.");
        replyMSG.append("<table>");
        replyMSG.append("<tr><td width=100>內容</td><td>修改</td></tr>");
        replyMSG.append("<tr><td><br></td><td></td></tr>");
        replyMSG.append("<tr><td>物品編號</td><td><edit var=\"itemID\" width=80></td></tr>");
    ...
    private TextBuilder itemListHtml(L2TradeList tradeList, int page)
    {
        ...
        replyMSG.append("<html><title>商人販賣列表: "+page+"</title>");
        replyMSG.append("<body>");
        replyMSG.append("<br>修改增加與刪除登錄值在商人列表");
        replyMSG.append("<table>");
        ...
        for (L2ItemInstance item : tradeList.getItems(start, end+1))
        {
            ...
            replyMSG.append("<td><button value=\"刪\" action=\"bypass -h admin_delShopItem "+tradeList.getListId()+" "+item.getItemId()+"\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
            replyMSG.append("</tr>");
        }//*/
        replyMSG.append("<tr>");
        int min = 1;
        ...
        if (page > 1)
        {
            replyMSG.append("<td><button value=\"頁數"+(page - 1)+"\" action=\"bypass -h admin_showShopList "+tradeList.getListId()+" "+(page - 1)+"\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
        }
        if (page < max)
        {
            if (page <= min)
    ...
    private void showShop(L2PcInstance admin, int merchantID)
    {
        ...
        if(tradeLists == null)
        {
        	...
        	admin.sendMessage("未知的NPC基本編號" + merchantID);
            return ;
        }
        ...
        TextBuilder replyMSG = new TextBuilder("<html><title>商人販賣列表</title>");

        replyMSG.append("<body>");
        replyMSG.append("<br>選擇列表檢視");
        replyMSG.append("<table>");
        replyMSG.append("<tr><td>商人列表編號</td></tr>");
...
//	    NpcTable.getInstance().replaceTemplate(template);
	...
	private void showNpcDropList(L2PcInstance admin, int npcId)
	{
	    ...
	    replyMSG.append("<body>");
	    replyMSG.append("<br>注意: 點下[drop_id]會顯示資料,點下[刪]會刪除掉落資料!");
	    replyMSG.append("<table>");

	    replyMSG.append("<tr><td>NPC 物品 類型</td><td>編號</td><td>種類</td><td>刪</td></tr>");
	...
	private void showEditDropData(L2PcInstance admin, int npcId, int itemId, int category)
	{
	    ...
	    try
	    {
	        ...
	        if(dropData.next()){
	            ...
	            replyMSG.append("<tr><td>物品名稱</td><td>"+ ItemTable.getInstance().getTemplate(dropData.getInt("itemId")).getName() + "(" + dropData.getInt("itemId") + ")</td></tr>");
	            ...
	            replyMSG.append("<center>");
	            replyMSG.append("<button value=\"存取修改\" action=\"bypass -h admin_edit_drop " + npcId + " " + itemId + " " + category +" $min $max $chance\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
	            replyMSG.append("<br><button value=\"掉落資料\" action=\"bypass -h admin_show_droplist " + dropData.getInt("mobId") +"\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
	            replyMSG.append("</center>");
	        }
	...
	{	    
        ...
        replyMSG.append("<table>");
        replyMSG.append("<tr><td>物品編號</td><td><edit var=\"itemId\" width=80></td></tr>");
        replyMSG.append("<tr><td>最小數量</td><td><edit var=\"min\" width=80></td></tr>");
        replyMSG.append("<tr><td>最大數量</td><td><edit var=\"max\" width=80></td></tr>");
        replyMSG.append("<tr><td>自體變化(-1)</td><td><edit var=\"category\" width=80></td></tr>");
        replyMSG.append("<tr><td>掉落機率(0-1000000)</td><td><edit var=\"chance\" width=80></td></tr>");

        replyMSG.append("</table>");

        replyMSG.append("<center>");
        replyMSG.append("<button value=\"存取資料\" action=\"bypass -h admin_add_drop " + npcData.npcId + " $itemId $category $min $max $chance\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
        replyMSG.append("<br><button value=\"掉落資料\" action=\"bypass -h admin_show_droplist " + npcData.npcId +"\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
        replyMSG.append("</center>");
        replyMSG.append("</body></html>");
	    adminReply.setHtml(replyMSG.toString());
	...
	private void updateDropData(L2PcInstance admin, int npcId, int itemId, int min, int max, int category, int chance)
	{
	    ...
	    try
	    {
	        ...
	        if(npcId > 0)
	        {
		        ...
		        replyMSG.append("<body>");
		        replyMSG.append("<center><button value=\"掉落列表\" action=\"bypass -h admin_show_droplist "+ npcId + "\" width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></center>");
		        replyMSG.append("</body></html>");
	...
	private void addDropData(L2PcInstance admin, int npcId, int itemId, int min, int max, int category, int chance)
	{
	    ...
	    try
	    {
            ...
            TextBuilder replyMSG = new TextBuilder("<html><title>掉落增加成功</title>");

	        replyMSG.append("<body>");
	        replyMSG.append("<center><button value=\"繼續增加\" action=\"bypass -h admin_add_drop "+ npcId + "\" width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
	        replyMSG.append("<br><br><button value=\"掉落列表\" action=\"bypass -h admin_show_droplist "+ npcId + "\" width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
	        replyMSG.append("</center></body></html>");
	...
	private void deleteDropData(L2PcInstance admin, int npcId, int itemId, int category)
	{
	    ...
	    try
	    {
	        ...
	        if(npcId > 0)
	        {
    	        ...
    	        TextBuilder replyMSG = new TextBuilder("<html><title>刪除掉落資料(" + npcId+", "+ itemId+", "+ category + ")完成</title>");
    	        replyMSG.append("<body>");
    	        replyMSG.append("<center><button value=\"掉落列表\" action=\"bypass -h admin_show_droplist "+ npcId + "\" width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></center>");
    	        replyMSG.append("</body></html>");
