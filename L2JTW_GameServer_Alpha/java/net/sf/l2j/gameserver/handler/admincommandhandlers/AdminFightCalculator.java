...
public class AdminFightCalculator implements IAdminCommandHandler {
	...
	private void handleStart(String params, L2PcInstance activeChar) {
		...
		{
            replyMSG.append("<html><title>請選擇NPC進行戰鬥</title>");
            replyMSG.append("<body>");
            replyMSG.append("<table>");
            replyMSG.append("<tr><td>第一位</td><td>第二位</td></tr>");
            replyMSG.append("<tr><td>等級 "+lvl1+"</td><td>等級 "+lvl2+"</td></tr>");
            replyMSG.append("<tr><td>編號 "+npc1.npcId+"</td><td>編號 "+npc2.npcId+"</td></tr>");
            replyMSG.append("<tr><td>"+npc1.name+"</td><td>"+npc2.name+"</td></tr>");
            replyMSG.append("</table>");
            replyMSG.append("<center><br><br><br>");
            replyMSG.append("<button value=\"開始戰鬥\" action=\"bypass -h admin_fight_calculator_show "+npc1.npcId+" "+npc2.npcId+"\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
		...
		else if (lvl2 != 0 && npc2 == null)
		{
            ...
            replyMSG.append("<body>");
            replyMSG.append("<table>");
            replyMSG.append("<tr><td>請輸入NPC等級來尋找NPC</td></tr>");
            replyMSG.append("<tr><td>第一位</td><td>第二位</td></tr>");
            replyMSG.append("<tr><td><edit var=\"lvl1\" width=80></td><td><edit var=\"lvl2\" width=80></td></tr>");
            replyMSG.append("</table>");
            replyMSG.append("<center><br><br><br>");
            replyMSG.append("<button value=\"選擇完畢\" action=\"bypass -h admin_fight_calculator lvl1 $lvl1 lvl2 $lvl2\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
            replyMSG.append("</center>");
            replyMSG.append("</body></html>");
		}
	...
	private void handleShow(String params, L2PcInstance activeChar) {
		...
		TextBuilder replyMSG = new TextBuilder();
        replyMSG.append("<html><title>戰鬥結果</title>");
        replyMSG.append("<body>");
        replyMSG.append("<table>");
        if (params.length() == 0) {
            replyMSG.append("<tr><td width=140>測試者</td><td width=70>我方</td><td width=70>目標</td></tr>");
        } else {
            replyMSG.append("<tr><td width=140>測試者</td><td width=70>"+((L2NpcTemplate)npc1.getTemplate()).name+
                    "</td><td width=70>"+((L2NpcTemplate)npc2.getTemplate()).name+"</td></tr>");
        }
        replyMSG.append("<tr><td>攻擊失敗</td><td>"+miss1+"%</td><td>"+miss2+"%</td></tr>");
        replyMSG.append("<tr><td>盾牌擋下</td><td>"+shld2+"%</td><td>"+shld1+"%</td></tr>");
        replyMSG.append("<tr><td>致命攻擊</td><td>"+crit1+"%</td><td>"+crit2+"%</td></tr>");
        replyMSG.append("<tr><td>物攻 / 物防</td><td>"+((int)patk1)+" / "+((int)pdef1)+"</td><td>"+((int)patk2)+" / "+((int)pdef2)+"</td></tr>");
        replyMSG.append("<tr><td>造成傷害</td><td>"+sAtk1+"</td><td>"+sAtk2+"</td></tr>");
        replyMSG.append("<tr><td>平均傷害</td><td>"+((int)dmg1)+"</td><td>"+((int)dmg2)+"</td></tr>");
        replyMSG.append("<tr><td>受到傷害</td><td>"+tdmg2+"</td><td>"+tdmg1+"</td></tr>");
        replyMSG.append("<tr><td>回覆血量</td><td>"+hp1+"</td><td>"+hp2+"</td></tr>");
        replyMSG.append("<tr><td>最大血量</td><td>"+(int)maxHp1+"</td><td>"+(int)maxHp2+"</td></tr>");
        replyMSG.append("<tr><td>死亡時間</td>");
        if (tdmg2 - hp1 > 1)
            replyMSG.append("<td>"+(int)(100*maxHp1/(tdmg2 - hp1))+" 秒</td>");
        else
            replyMSG.append("<td>永遠不死</td>");
        if (tdmg1 - hp2 > 1)
            replyMSG.append("<td>"+(int)(100*maxHp2/(tdmg1 - hp2))+" 秒</td>");
        else
            replyMSG.append("<td>永遠不死</td>");
		replyMSG.append("</tr>");
        ...
        if (params.length() == 0) {
            replyMSG.append("<button value=\"重新嘗試\" action=\"bypass -h admin_fight_calculator_show\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
        } else {
            replyMSG.append("<button value=\"重新嘗試\" action=\"bypass -h admin_fight_calculator_show "+((L2NpcTemplate)npc1.getTemplate()).npcId+" "+((L2NpcTemplate)npc2.getTemplate()).npcId+"\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
        }
        replyMSG.append("</center>");
        replyMSG.append("</body></html>");
        adminReply.setHtml(replyMSG.toString());
