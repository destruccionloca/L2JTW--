...
public final class Formulas
{
										...
										boolean ss, boolean bss, boolean mcrit)
	{
		...
		//double damage = 81*Math.sqrt(mAtk)/mDef*skill.getPower(attacker)*0.75;
//		if(attacker instanceof L2PcInstance && target instanceof L2PcInstance) damage *= 0.9; // PvP modifier (-10%)
		...
		if (Config.ALT_GAME_MAGICFAILURES && !calcMagicSuccess(attacker, target, skill))
		{
			...
			if (attacker instanceof L2PcInstance)
			{
				...
				{
					if (skill.getSkillType() == SkillType.DRAIN) attacker.sendPacket(new SystemMessage(SystemMessage.DRAIN_HALF_SUCCESFUL));
