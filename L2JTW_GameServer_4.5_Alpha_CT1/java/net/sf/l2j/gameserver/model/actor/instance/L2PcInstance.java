...
public final class L2PcInstance extends L2PlayableInstance
{
	...
	*/
    ...
    public ScheduledFuture<?> _posCheckerCTF = null;
	//---------------------------------------
...
//	public boolean isInCraftMode() { return (getPrivateStoreType() == STORE_PRIVATE_MANUFACTURE); }
    ...
    public void checkIfWeaponIsAllowed()
    {
        ...
        for (L2Effect currenteffect : getAllEffects())
        {
            ...
            if (!effectSkill.isOffensive() && !(effectSkill.getTargetType() == SkillTargetType.TARGET_PARTY && effectSkill.getSkillType() == SkillType.BUFF))
            {
                ...
                {
                    sendMessage(effectSkill.getName() + "無法在此種類武器上使用.");
	...
	private void giveAvailableSkills()
	{
		...
		sendMessage("習得" + skillCounter + "種新技能！");
	}
	...
	public void standUp()
	{
		...
		{
			sendMessage("一股未知的黑暗力量讓腳發抖起來，導致無法站起。");
		}
    ...
    public void transformInsertInfo()
    {
        ...
        {
            con = L2DatabaseFactory.getInstance().getConnection();
            statement = con.prepareStatement("UPDATE characters SET transform_id=? WHERE charId=?");
        ...
        else
        {
            ...
            if(ptarget!=null)
            {
                if (answer == 1)
                {
                    ...
                    ptarget.sendMessage("邀請結婚被 >接受< 了");
                }
                else
                    ptarget.sendMessage("邀請結婚被 >拒絕< 了!");
            }
        }
    }
	...
	{
		if (atEvent || killer == null || (CTF._started && _inEventCTF))
            return;
		if (getKarma()<=0
...
//                      || this.getClan().isAtWarWith(((L2PcInstance)killer).getClanId())
	...
	public void deathPenalty(boolean atwar, boolean killed_by_pc)
	{
		...
		long lostExp = 0;
		if (!atEvent)
			if (lvl < Experience.MAX_LEVEL||(_inEventCTF && !CTF._started) || (!_inEventCTF && CTF._started) || (!_inEventCTF && CTF._started))
	...
	public void setChatBanned(boolean isBanned)
	{
		...
		if (isChatBanned())
			sendMessage("您被遊戲管理員給封鎖了.");
		else
		{
			sendMessage("您被遊戲管理員解除封鎖了.");
	...
	public void onPlayerEnter()
	{
        ...
        if (SevenSigns.getInstance().isSealValidationPeriod() || SevenSigns.getInstance().isCompResultsPeriod())
        {
            if (!isGM() && isIn7sDungeon() && SevenSigns.getInstance().getPlayerCabal(this) != SevenSigns.getInstance().getCabalHighestScore())
            {
                ...
                sendMessage("因為目前為封印有效時期,將強制傳送至村莊.");
            }
        }
        else
        {
            if (!isGM() && isIn7sDungeon() && SevenSigns.getInstance().getPlayerCabal(this) == SevenSigns.CABAL_NULL)
            {
                teleToLocation(MapRegionTable.TeleportWhereType.Town);
                setIsIn7sDungeon(false);
                sendMessage("因為沒有參與任何封印,將強制傳送至村莊.");
            }
        }
        ...
        if (_isInvul)
        	sendMessage("目前為無法傷害狀態.");
        if (getAppearance().getInvisible())
            sendMessage("目前為隱形狀態.");
        if (getMessageRefusal())
            sendMessage("目前為訊息禁止狀態.");
    ...
    public void setInJail(boolean state, int delayInMinutes)
    {
        ...
        if (_inJail)
        {
            ...
            if (jailInfos != null)
            	...
            	htmlMsg.setHtml("<html><body>被管理員傳送送入監獄.</body></html>");
            sendPacket(htmlMsg);
        ...
        } else
        {
            ...
            if (jailInfos != null)
            	...
            	htmlMsg.setHtml("<html><body>監獄解除,請多遵守伺服器規定!</body></html>");
            sendPacket(htmlMsg);
    ...
    private void updateJailState()
    {
        if (isInJail())
        {
            ...
            if (_jailTimer > 0)
            {
                ...
                _jailTask = ThreadPoolManager.getInstance().scheduleGeneral(new JailTask(this), _jailTimer);
                sendMessage("監獄時間還有 "+Math.round(_jailTimer/60000)+" 分鐘.");
            }
...
//L2JTW addon end ==========================================
 // L2JTW Addon start ======================================
    ...
    {
        if (Config.DEBUG)
            _log.fine("UserInfo Force Update");
        // Send a Server->Client packet UserInfo to this L2PcInstance
        sendPacket(new UserInfo(this));
