...
public class AdminEffects implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_earthquake"))
		{
			...
			{
				activeChar.sendMessage("使用方法: //earthquake <intensity> <duration>");
			}
		}
			...
			try
			{
				...
				for (L2PcInstance player : activeChar.getKnownList().getKnownPlayers().values())
				{
					if (activeChar.isInsideRadius(player, 400, false, true))
					{
						...
						if (teamVal != 0)
						{
							player.sendMessage("加入"+ teamVal+"隊伍。");
						}
						player.broadcastUserInfo();
					}
				}
		...
		else if (command.startsWith("admin_social"))
		{
			try
			{
				...
				else if (st.countTokens() == 1)
				{
					...
					activeChar.sendMessage("使用方法: //social <social_id> [player_name|radius]");
			}
			catch (Exception e)
			{
				if (Config.DEBUG)
		...
		else if (command.startsWith("admin_effect"))
		{
			try
			{
				...
				if (obj != null)
				{
					...
					else
					{
						...
						activeChar.sendMessage(obj.getName()+" 使用MSU "+skill+"/"+level+".");
					}
				}
				else
					activeChar.sendPacket(new SystemMessage(SystemMessageId.INCORRECT_TARGET));
			...
			{
				activeChar.sendMessage("使用方法: //effect skill [level | level hittime]");
			}
		}
	...
	private void adminAtmosphere(String type, String state, L2PcInstance activeChar)
	{
		...
		else if(type.equals("sky"))
		{
			...
			activeChar.sendMessage("使用方法: //atmosphere <signsky dawn|dusk>|<sky day|night|red>");
