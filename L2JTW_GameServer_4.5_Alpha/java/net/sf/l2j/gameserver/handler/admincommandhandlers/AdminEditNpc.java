...
public class AdminEditNpc implements IAdminCommandHandler {
	...
	{//TODO: Tokenize and protect arguments parsing. Externalize HTML.
		...
		else if(command.startsWith("admin_edit_drop "))
		{
			...
			try
			{
				...
				else if (st.countTokens() == 6)
				{
					...
					activeChar.sendMessage("使用方法: //edit_drop <npc_id> <item_id> <category> [<min> <max> <chance>]");
		...
		else if (command.startsWith("admin_add_drop "))
		{
			...
			try
			{
				...
				else if(st.countTokens() == 6)
				{
					...
					activeChar.sendMessage("使用方法: //add_drop <npc_id> [<item_id> <category> <min> <max> <chance>]");
		...
		else if(command.startsWith("admin_del_drop "))
		{
			...
			if(npcId > 0)
				...
				activeChar.sendMessage("使用方法: //del_drop <npc_id> <item_id> <category>");
	...
	private void addShopItem(L2PcInstance activeChar, String[] args)
	{
		...
		if (args.length > 3)
		{
			...
			activeChar.sendMessage("增加 "+newItem.getItem().getName()+" 至商店資料 "+tradeList.getListId());
			showShopList(activeChar, tradeListID, 1);
	...
	private void showNpcDropList(L2PcInstance activeChar, int npcId)
	{
		...
		if(npcData == null)
		{
			activeChar.sendMessage("編號錯誤" + npcId);
			return ;
		}
