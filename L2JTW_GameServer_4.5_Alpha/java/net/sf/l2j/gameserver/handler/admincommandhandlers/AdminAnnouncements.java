...
public class AdminAnnouncements implements IAdminCommandHandler {
	...
	private static final String[] ADMIN_COMMANDS = {
			...
			"admin_one_announcement",//add by game
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        ...
        else if (command.startsWith("admin_one_announcement"))
        {
            /** 加入單行公告處理 ADD BY Game*/
            int val = new Integer(command.substring(23)).intValue();
            for (L2PcInstance player : L2World.getInstance().getAllPlayers())
            {
                Announcements.getInstance().showOneAnnouncement(player,val);
            }
            Announcements.getInstance().listAnnouncements(activeChar);
        }
