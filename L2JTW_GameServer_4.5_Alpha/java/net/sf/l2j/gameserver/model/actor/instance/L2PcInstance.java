...
import net.sf.l2j.gameserver.serverpackets.UserInfoMultiplier;
...
//	public boolean isInCraftMode() { return (getPrivateStoreType() == STORE_PRIVATE_MANUFACTURE); }
		...
		if (_inMotherTreeZone != inMotherTreeZone)
		{
			...
			if (warnPlayer)
				if (_inMotherTreeZone)
					sendPacket(new SystemMessage(SystemMessageId.ENTER_SHADOW_MOTHER_TREE));
				else
					sendPacket(new SystemMessage(SystemMessageId.EXIT_SHADOW_MOTHER_TREE));
		}
	}
	...
	public final void broadcastUserInfoMultiplier()
	{
		_log.warning("UserInfo Force Update");
		// Send a Server->Client packet UserInfo to this L2PcInstance
		sendPacket(new UserInfoMultiplier(this));

		// Send a Server->Client packet CharInfo to all L2PcInstance in _KnownPlayers of the L2PcInstance
		if (Config.DEBUG)
            _log.fine("players to notify:" + getKnownList().getKnownPlayers().size() + " packet: [S] 03 CharInfo");

		Broadcast.toKnownPlayers(this, new CharInfoMultiplier(this));
	}
...
//                      || this.getClan().isAtWarWith(((L2PcInstance)killer).getClanId())
	...
	{
		_log.warning("UpdateStats in PcInstance");
	...
	{
		//there is no need to send the UserInfo 2 times...
		//sendPacket(new UserInfo(this));
		for (L2PcInstance player : getKnownList().getKnownPlayers().values()) {
			player.sendPacket(new RelationChanged(this, getRelation(player), isAutoAttackable(player)));
		}
...
/*
         if (isCastingNow())//add skill to the Quen
             {
             _nextSkill = skill;
             _fu = forceUse;
             _dm = dontMove;
      ...
      // If a skill is currently being used, queue this one if this is not the same
        ...
        if (skill.getItemConsume() > 0)
        {
            ...
            if (requiredItems == null || requiredItems.getCount() < skill.getItemConsume())
            {
            	...
            	// Useless and Error command due to confuse of Cubic summon and Sevritor ...
            	// People who do System Message should seriuosly use their brain before doing some dumb stuff..
            	/*
                ...
                */
            	...
            	// More correctly and easier to understand....widly use for skill...
...
/*
    public L2Skill GetNextSkill()
     {
     return _nextSkill;
     }
     public void SetNextSkill(L2Skill s)
     {
     _nextSkill = s;
     }
     public boolean GetNextSkillFU()
     {
     return _fu;
     }
     public boolean GetNextSkillDM()
     {
     return _dm;
     }
*/
