...
//	public boolean isInCraftMode() { return (getPrivateStoreType() == STORE_PRIVATE_MANUFACTURE); }
    ...
    public void CheckIfWeaponIsAllowed()
    {
        ...
        { 
            ...
            if (!(effectSkill.getTargetType() == SkillTargetType.TARGET_PARTY && effectSkill.getSkillType() == SkillType.BUFF))
            {
                ...
                {
                    sendMessage(effectSkill.getName() + "µLªk¨Ï¥Î©ó¦¹ªZ¾¹.");
	...
	public void rewardSkills()
	{
        ...
        if (Config.AUTO_LEARN_SKILLS) {
            ...
            int unLearnable = 0;
            int skillCounter = 0;
            L2SkillLearn[] skills = SkillTreeTable.getInstance().getAvailableSkills(this, this.getClassId());
            while(skills.length > unLearnable)
            {
                for (L2SkillLearn s : skills)
                {
                    L2Skill sk = SkillTable.getInstance().getInfo(s.getId(), s.getLevel());
                    if (sk == null || !sk.getCanLearn(this.getClassId()))
                    {
                        if(countUnlearnable)
                            unLearnable++;
                        continue;
                ...
                skills = SkillTreeTable.getInstance().getAvailableSkills(this, this.getClassId());
            }
            this.sendMessage("¾Ç²ß¤F " + skillCounter + " ­Ó§Þ¯à.");
        }
	...
	public void standUp()
	{
		...
		{
			sendMessage("¤@­Ó¶Â·tªº¤O¶q»·¶W¶V¥Íª«¯à·Q¹³ªº±N§Aªºµo§Ý..");
		}
...
//			statement.setInt(30, getBaseTemplate().race.ordinal());
	...
	public boolean addHenna(L2HennaInstance henna)
	{
		...
		{
			this.sendMessage("¯¾¨­µLªk¸Ë³Æ¶W¹L¤T­Ó¥H¤W");
			return false;
		}
	...
	public void onPlayerEnter()
	{
        ...
        if (SevenSigns.getInstance().isSealValidationPeriod() || SevenSigns.getInstance().isCompResultsPeriod())
        {
            if (!isGM() && isIn7sDungeon() && SevenSigns.getInstance().getPlayerCabal(this) != SevenSigns.getInstance().getCabalHighestScore())
            {
                ...
                sendMessage("¥Ø«e¬°«Ê¦L¦³®Ä´Á­­­è¶}©l¡A±N¶Ç°e¦Ü¦U§ø²ø.");
            }
        }
        else
        {
            if (!isGM() && isIn7sDungeon() && SevenSigns.getInstance().getPlayerCabal(this) == SevenSigns.CABAL_NULL)
            {
                ...
                setIsIn7sDungeon(false);
                sendMessage("¨ÃµL°Ñ¥[¥ô¦ó¤C«Ê¦L,±N¶Ç°e¦Ü¦U§ø²ø.");
            }
        }
        ...
        if (getAppearance().getInvisible()) 
            sendMessage("Áô§Î¼Ò¦¡±Ò°Ê");
        if (getMessageRefusal()) 
            sendMessage("±K»yÀW¹DÃö³¬");
    ...
    public void setInJail(boolean state, int delayInMinutes)
    {
        ...
        if (_inJail)
        {
            if (delayInMinutes > 0)
            {
                ...
                sendMessage("ºÊº»®É¶¡¬° "+delayInMinutes+" ¤ÀÄÁ.");
            }
            ...
            if (jailInfos != null)
                ...
                htmlMsg.setHtml("<html><body>¶i¤JºÊº»</body></html>");
            sendPacket(htmlMsg);
        ...
        } else
        {
            ...
            if (jailInfos != null)
                ...
                htmlMsg.setHtml("<html><body>ºÊº»®É¶¡¥H¨ì,½Ð¦h´L­«¦øªA¾¹.</body></html>");
            sendPacket(htmlMsg);
    ...
    private void updateJailState()
    {
        if (isInJail())
        {
            ...
            if (_jailTimer > 0)
            {
                ...
                _jailTask = ThreadPoolManager.getInstance().scheduleGeneral(new JailTask(this), _jailTimer);
                sendMessage("¥Ø«eºÊº»®É¶¡¬° "+Math.round(_jailTimer/60000)+" ¤À.");
            }
