...
public abstract class L2Character extends L2Object
{
	...
	public boolean _requirePacketSend = false;//it tells when it need to send packet for buff
	public boolean _requireBroadcast = false;// it tells when it need to broadcast instead a normal buffs( for Multiplier )
...
//  =============================================================================================================================================================
    ...
    public final void setHeadingTo(L2Character target)
    {
        if (target == null || target == this) return;
        _heading = (int) (Math.atan2(getY() - target.getY(),getX() - target.getX()) * 32768./Math.PI + 32768);
    }
    
    /** Get the heading of the attacker to target **/
    public int getHeadingTo(L2Character target, boolean toChar)
    {
        if (target == null || target == this) return -1;
        
        int dx = target.getX() - getX();
        int dy = target.getY() - getY();
        int heading = (int) (Math.atan2(-dy,-dx) * 32768./Math.PI);
        
        if (toChar) heading = target.getHeading() - (heading + 32768);
        else heading = getHeading() - (heading + 32768);
    ...
    // TARGET AND ZONE CHECKS
//  =============================================================================================================================================================
    
     /** Returns true if the caster is positive to the target */
     public boolean isPositiveTo(L2Object obj)
     {
             if (    // Minions only Boss
                     (this instanceof L2MinionInstance && obj instanceof L2RaidBossInstance)
                     // Boss only Minions
                     || (this instanceof L2RaidBossInstance && obj instanceof L2MinionInstance)
                     // Monsters only Monsters
                     || ((this instanceof L2MonsterInstance && !(this instanceof L2MinionInstance)&& !(this instanceof L2RaidBossInstance))
                     && (obj instanceof L2MonsterInstance &&  !(obj instanceof L2MinionInstance) && !(obj instanceof L2RaidBossInstance)))
                     // SiegeGuards only SiegeGuards
                     || (this instanceof L2SiegeGuardInstance && obj instanceof L2SiegeGuardInstance)
                     // can self buff/heal
                     || obj == this
                 )
                 return true;
...
//   =============================================================================================================================================================
     ...
     /** Returns true if the attacker is not allowed to hit the target */
     public boolean isNotAllowedTo(L2Object obj)
     {
         // initialize check for party/clan
         L2PcInstance character = null;
         L2Party party = null;
         L2Clan clan = null;
         L2Party objParty = null;
         L2Clan objClan = null;
         
         if (this instanceof L2PcInstance) character = (L2PcInstance)this;
         if (this instanceof L2Summon) character = ((L2Summon)this).getOwner();
         
         if (obj instanceof L2PcInstance) { objParty = ((L2PcInstance)obj).getParty(); objClan = ((L2PcInstance)obj).getClan(); }
         if (obj instanceof L2Summon) { objParty = ((L2Summon)obj).getParty(); objClan = ((L2Summon)obj).getOwner().getClan(); }
		...
		if (!miss1)
		{
			...
			if (this instanceof L2Attackable)
	        {
	            if (((L2Attackable)this).getSoulShot() >0)
	            {
	                ...
	                damage1 = (int)Formulas.getInstance().calcPhysDam(this, target, null, shld1, crit1, false, true);
	            }
	            else
	            damage1 = (int)Formulas.getInstance().calcPhysDam(this, target, null, shld1, crit1, false, false);
	        }
	        else
		...
		if (!miss2)
		{
			...
			if (this instanceof L2Attackable)
	        {
	            if (((L2Attackable)this).getSoulShot() >0)
	            {
	                ...
	                Broadcast.toSelfAndKnownPlayersInRadius(this, new MagicSkillUser(this, this, 2154, 1, 0, 0), 360000);
	                damage2 = (int)Formulas.getInstance().calcPhysDam(this, target, null, shld1, crit1, false, true);
	            }
	            else
	            damage2 = (int)Formulas.getInstance().calcPhysDam(this, target, null, shld1, crit1, false, false);
	        }
	        else
	...
	public void doCast(L2Skill skill)
	{
				...
				skill.getTargetType() == L2Skill.SkillTargetType.TARGET_ALLY)
		{
			...
			{
				if(skill.getSkillType() == SkillType.BUFF || skill.getSkillType() == SkillType.HOT || skill.getSkillType() == SkillType.HEAL || skill.getSkillType() == SkillType.HEAL_PERCENT || skill.getSkillType() == SkillType.MANAHEAL || skill.getSkillType() == SkillType.MANAHEAL_PERCENT || skill.getSkillType() == SkillType.BALANCE)
		...
		//_log.warning("Debug: Aura Skill Event 2");
		...
		//======================================
		// To prevent area skill animation/packet that server still not send out and cause delay
		
		  if(skill.getCondition() != 16 || skill.getCondition() != 17)
               broadcastPacket(new MagicSkillLaunched(this, magicId, level, targets));
                
		//======================================
		
		 // _log.warning("Debug: Aura Skill Event 3");
		...
		//_log.warning("Debug: Aura Skill Event 4");
	}
	...
	*/
	...
	public boolean isAttackingDisabled() { return isStunned() || isSleeping() || _attackEndTime > GameTimeController.getGameTicks() || isFakeDeath() || isParalyzed() || _castEndTime > GameTimeController.getGameTicks(); }
	...
	public boolean isMovementDisabled() { return isStunned() || isRooted() || isSleeping() || isOverloaded() || isParalyzed() || isImobilised() || isFakeDeath() || _attackEndTime > GameTimeController.getGameTicks() || _castEndTime > GameTimeController.getGameTicks();  }
...
//
	...
	protected void onHitTimer(L2Character target, int damage, boolean crit, boolean miss, boolean soulshot, boolean shld)
	{
		...
		if (!isAttackAborted())
		{
			...
			if (target.isRaid())
			{
				...
				if(!Config.ALT_RAIDBOSS_FOSSILIZATION)
				...
				if (target instanceof L2Character)
				{
					...
					if (activeWeapon != null && !((L2Character)target).isDead())
					{
						if (activeWeapon.getSkillEffects(this, player, skill).length > 0 && this instanceof L2PcInstance)
						{
							...
							sendPacket(SystemMessage.sendString("武器特殊效果並沒影響到對方!"));
						}
					}
	...
	{
		//_log.warning("Update Status which is Not Implement yet");
	}
