	    ...
	    //_log.warning(activeChar.getName()+" request restartpoint "+requestedPointType);
            try
            {
                ...
                else if (requestedPointType == 1) // to clanhall
                {
                    ...
                    {
                        //cheater
                        activeChar.sendMessage("請勿使用任何BUG.");
                        Util.handleIllegalPlayerAction(activeChar, "Player " + activeChar.getName()
                                                       + " 嘗試使用返回BUG.", IllegalPlayerAction.PUNISH_KICK);
                        return;
                    }
    ...
    protected void runImpl()
    {
        ...
        if (castle != null && castle.getSiege().getIsInProgress())
        {
                    ...
                    && castle.getSiege().checkIsAttacker(activeChar.getClan()))
            {
                ...
                sm.addString("SYS");
                sm.addString("將在 " + castle.getSiege().getAttackerRespawnDelay()/1000 + " 秒後返回");
            ...
            else
            {
                ...
                sm.addString("將在 " + castle.getSiege().getDefenderRespawnDelay()/1000 + " 秒內返回");
                activeChar.sendPacket(sm);
            }
            sm = null;
            return;
